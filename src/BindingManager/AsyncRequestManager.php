        ...
        while ($done = curl_multi_info_read($this->multi_handle)) {
            ...
            if ($requestInfo['callback'] !== null) {
                if ($error !== CURLE_OK || $httpCode !== 200) {
                    Server::getInstance()->getLogger()->debug("[BindingManager] cURL request failed with error {$error} and HTTP code {$httpCode}. Response: " . ($response === false ? "false" : (string)$response));
